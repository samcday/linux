 ...
 */
enum tfa_error tfa_supported_speakers(struct tfa_device *tfa,
					  int *spkr_count)
{
	if (tfa->in_use == 0)
...
static enum tfa_error tfa98xx_faim_protect(struct tfa_device *tfa, int state)
{
...
enum tfa_error tfa98xx_dsp_write_tables(struct tfa_device *tfa,
					    int sample_rate)
{
	enum tfa_error error = tfa_error_ok;
...
enum tfa_error tfa98xx_dsp_get_memory(struct tfa_device *tfa,
...
enum tfa_error tfa98xx_dsp_set_memory(struct tfa_device *tfa,
					  int memoryType, int offset,
...
static enum tfa_error tfa98xx_set_mute_tfa2(struct tfa_device *tfa,
...
enum tfa_error tfa_dsp_msg_write_id(struct tfa_device *tfa, int length,
 ...
 */
enum tfa_error tfa_dsp_msg_id(struct tfa_device *tfa, int length,
...
enum tfa_error tfa98xx_dsp_get_state_info(struct tfa_device *tfa,
					      unsigned char bytes[],
...
enum tfa_error tfa98xx_dsp_support_framework(struct tfa_device *tfa,
						 int *pbSupportFramework)
{
	...
	/* *pbSupportFramework only changed when error == tfa_error_ok */
	return error;
}
 ...
 */
enum tfa_error tfaRunWaitCalibration(struct tfa_device *tfa,
					 int *calibrateDone)
{
	enum tfa_error err = tfa_error_ok;
...
/* Check if MTP_busy is clear! */
		...
		if (tries_mtp_busy < MTPBWAIT_TRIES) {
			/* Because of the msleep TFA98XX_API_WAITRESULT_NTRIES is way to long!
				* Setting this to 25 will take it at least 25*50ms = 1.25 sec
				*/
	...
	if ((err != tfa_error_ok) && ((tfa->rev & 0x0FFF) == 0xc88)) {
 ...
 */
enum tfa_error tfa_dsp_get_calibration_impedance(struct tfa_device *tfa)
{
	enum tfa_error error = tfa_error_ok;

...
static enum tfa_error tfaContWriteDrcFile(struct tfa_device *tfa, int size,
				       uint8_t data[])
{
	enum tfa_error err = tfa_error_ok;
	uint8_t *msg = NULL;
					       ...
					       int fs_previous_profile)
{
	...
	*/
	...
	if (fs_next_profile != fs_default_profile) {
		 ...
		 */
		err = tfa->dev_ops.dsp_write_tables(tfa, fs_next_profile);
		 ...
		 */
		err = tfa->dev_ops.dsp_write_tables(tfa, fs_default_profile);
	}
